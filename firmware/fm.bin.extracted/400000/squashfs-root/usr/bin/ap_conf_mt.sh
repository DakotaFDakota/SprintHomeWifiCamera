...
echo "interface=ra0" > $APCONF_PATH
...
set_ap_conf()
{
	MACAddr=`ifconfig ra0 | grep 'HWaddr ' | cut -d' ' -f10 | awk '{ print $1 }'`
	SSID=`$NV_GET wireless.ssid`
	echo "ignore_broadcast_ssid=0" >> $APCONF_PATH
	echo "ssid=$SSID$MACAddr" >> $APCONF_PATH
...
set_udhcpd_conf()
{
	...
	echo "interface ra0" > $UDHCPDCONF_PATH
