...
function radioTable(fObj,radioObj,act_str)
{
	...
	else alert(msg_noradio_selected);
}
...
function dataToAdd(form_obj)  // both hidden & visible fields in same form
{
	var form_size = form_obj.elements.length;
	var destField; 	var last_name; 	 var radioIndex;  var baseRef;
...
function dataToEnable(form_obj)  // both hidden & visible fields in same form
{
	var form_size = form_obj.elements.length;
	var destField; 	var last_name; 	 var radioIndex;  var baseRef;
...
// creates matching hidden form fields - used for developement
{
	var form_size = form_obj.elements.length;
	...
	{
		open();
		writeln('<html><he' + 'ad><tit' + 'le>Form Definition</ti' + 'tle></he' + 'ad>');
		...
		for (var i = 0; i < form_size; i++)
		{
			...
			else if (form_obj.elements[i].type == "checkbox")
			{
				...
				hfieldStr += '\n<input type="hidden" name="h_' + form_obj.elements[i].name + '" value="@' + form_obj.elements[i].name + '#">';
				hdemoStr += '\n<input type="hidden" name="h_' + form_obj.elements[i].name + '" value="1">';
			}
			else ;  // no action
		}
		writeln('\n<!-- DemoStart -->' + hdemoStr + '\n<!-- DemoEnd -->');
