...
{ 
    ...
    { 
    		//alert("activexObjectName="+activexObjectName);
        try 
        { 
...
function detect_IE_by_ClassID(ClassID)
{ 
    var result = false; 

    document.write('<SCRIPT LANGUAGE=VBScript>\n on error resume next \n result = IsObject(CreateObject("' + ClassID + '"))</SCRIPT>\n'); 
    //alert("detect_IE_by_ClassID: ClassID="+ClassID+",result="+result);
    if (result) 
        return true; 
    else return false; 
}

function detect_NS_by_ClassID(ClassID) 
{ 
    if (nse.indexOf(ClassID) != -1) 
    {
    	if (navigator.mimeTypes[ClassID].enabledPlugin != null) 
    	{
            //alert("detect_NS_by_ClassID: ClassID="+ClassID+",result=true");
    		return true;
    	}
    }
    //alert("detect_NS_by_ClassID: ClassID="+ClassID+",result=false");
    return false;
}
function detect_NS_by_Name(name) 
{
    ...
    for (var i=0; i < num_of_plugins; i++) 
    {
        ...
        {
            //alert("detect_NS_by_Name: name="+name+",result=true");
            return true;
        }
    }
    //alert("detect_NS_by_Name: name="+name+",result=false");
    return false;
}

function dt_plug_in_by_ClassID_and_name(ie_classid, ns_class_id,name)
{
	if (!checkPlugins(ns_class_id,ie_classid))
	{
	    if (ie && win)
	    {
	        return detect_IE_by_ClassID(ie_classid);
	    }
	    else
	    {
	        if (detect_NS_by_ClassID(ns_class_id))
	        {
	            return true;
	        }
	        else
	        {
	            return detect_NS_by_Name(name);
	        }
	    }
  	}
  	else
...
function dt_quciktime()
{
    return dt_plug_in_by_ClassID_and_name("QuickTimeCheckObject.QuickTimeCheck.1","video/quicktime","QuickTime");
}
function dt_adobe_svg()
{
    return dt_plug_in_by_ClassID_and_name("Adobe.SVGCtl","image/svg-xml","SVG Viewer");
}
function dt_shockware_director()
{
    return dt_plug_in_by_ClassID_and_name("SWCtl.SWCtl.1","application/x-director","Shockwave Director");
}
function dt_shockwave_flash()
{
    return dt_plug_in_by_ClassID_and_name("ShockwaveFlash.ShockwaveFlash.1","application/x-shockwave-flash","Shockwave Flash");
}
function dt_realplayer()
{
    return dt_plug_in_by_ClassID_and_name("rmocx.RealPlayer G2 Control.1","audio/x-pn-realaudio-plugin","RealPlayer");
}
function dt_mediaplayer()
{
    return dt_plug_in_by_ClassID_and_name("MediaPlayer.MediaPlayer.1","application/x-mplayer2","Windows Media Player");
}
function dt_acrobat_reader()
{
    return dt_plug_in_by_ClassID_and_name("PDF.PdfCtrl.5","application/pdf","Acrobat Reader");
}
/*
function dt_java()
{
        if (detect_NS_by_ClassID("application/x-java-applet"))
        {
            return true;
        }
        else
        {
            return detect_NS_by_Name("Java");
        }
}
*/
...
return true;}else{return false;}}else{return deployJava.installLatestJRE();}},installLatestJRE:



function(){if(deployJava.isPluginInstalled()){if(deployJava.getPlugin().installLatestJRE()){deployJava.refresh();if(deployJava.returnPage!=null){document.location=deployJava.returnPage;}
...
minimumVersion+'&quot;)) {'+'if (deployJava.installLatestJRE()) {'+'if (deployJava.launch(&quot;'+jnlp+'&quot;)) {}'+'}'+'} else {'+'if (deployJava.launch(&quot;'+jnlp+'&quot;)) {}'+'}';document.write('<'+'a href="'+url+'" onMouseOver="window.status=\'\'; '+'return true;"><'+'img '+'src="'+deployJava.launchButtonPNG+'" '+'border="0" /><'+'/'+'a'+'>');},launch:function(jnlp){if(deployJava.isPluginInstalled()){return deployJava.getPlugin().launch(jnlp);}else{document.location=jnlp;return true;}},isPluginInstalled:




function(){var plugin=deployJava.getPlugin();if(plugin&&plugin.jvms){return true;}else{return false;}},isAutoUpdateEnabled:function(){if(deployJava.isPluginInstalled()){return deployJava.getPlugin().isAutoUpdateEnabled();}
return false;},setAutoUpdateEnabled:



function(){if(deployJava.isPluginInstalled()){return deployJava.getPlugin().setAutoUpdateEnabled();}
...
return false;},setEarlyAccess:function(enabled){deployJava.EAInstallEnabled=enabled;},isPlugin2:



function(){if(deployJava.isPluginInstalled()){if(deployJava.versionCheck('1.6.0_10+')){try{return deployJava.getPlugin().isPlugin2();}catch(err){}}}
return false;},getPlugin:
...
return true;}}else{return false;}},getBrowser:



function(){var browser=navigator.userAgent.toLowerCase();if(deployJava.debug){alert('userAgent -> '+browser);}
...
return false;}
try{return(new ActiveXObject(objectName)!=null);}catch(exception){return false;}},testForMSVM:



function(){var clsid='{08B0E5C0-4FCB-11CF-AAA5-00401C608500}';if(typeof oClientCaps!='undefined'){var v=oClientCaps.getComponentVersion(clsid,"ComponentID");if((v=='')||(v=='5,0,5000,0')){return false;}else{return true;}}else{return false;}},testUsingMimeTypes:function(version){if(!navigator.mimeTypes){if(deployJava.debug){alert('Browser claims to be Netscape family, but no mimeTypes[] array?');}
return false;}
...
return false;},IEInstall:



function(){location.href=deployJava.getJavaURL+
((deployJava.returnPage!=null)?('&returnPage='+deployJava.returnPage):'')+
((deployJava.locale!=null)?('&locale='+deployJava.locale):'')+
((deployJava.brand!=null)?('&brand='+deployJava.brand):'')+
((deployJava.installType!=null)?('&type='+deployJava.installType):'');return false;},done:function(name,result){},FFInstall:
...
if(a.length==2){a[2]=0;}
if(a[0]>b[0])return true;if(a[0]<b[0])return false;if(a[1]>b[1])return true;if(a[1]<b[1])return false;if(a[2]>b[2])return true;if(a[2]<b[2])return false;return true;},enableAlerts:



function(){deployJava.debug=true;},poll:function(){deployJava.refresh();var postInstallJREList=deployJava.getJREs();if((deployJava.preInstallJREList.length==0)&&(postInstallJREList.length!=0)){clearInterval(deployJava.myInterval);if(deployJava.returnPage!=null){location.href=deployJava.returnPage;};}
if((deployJava.preInstallJREList.length!=0)&&(postInstallJREList.length!=0)&&(deployJava.preInstallJREList[0]!=postInstallJREList[0])){clearInterval(deployJava.myInterval);if(deployJava.returnPage!=null){location.href=deployJava.returnPage;}}},writePluginTag:



function(){var browser=deployJava.getBrowser();if(browser=='MSIE'){document.write('<'+'object classid="clsid:CAFEEFAC-DEC7-0000-0000-ABCDEFFEDCBA" '+'id="deployJavaPlugin" width="0" height="0">'+'<'+'/'+'object'+'>');}else if(browser=='Netscape Family'){deployJava.writeEmbedTag();}},refresh:function(){navigator.plugins.refresh(false);var browser=deployJava.getBrowser();if(browser=='Netscape Family'){var plugin=document.getElementById('deployJavaPlugin');if(plugin==null){deployJava.writeEmbedTag();}}},writeEmbedTag:function(){var written=false;if(navigator.mimeTypes!=null){for(var i=0;i<navigator.mimeTypes.length;i++){if(navigator.mimeTypes[i].type==deployJava.mimeType){if(navigator.mimeTypes[i].enabledPlugin){document.write('<'+'embed id="deployJavaPlugin" type="'+
...
deployJava.oldMimeType+'" hidden="true" />');}}}}},do_initialize:




function(){deployJava.writePluginTag();if(deployJava.locale==null){var loc=null;if(loc==null)try{loc=navigator.userLanguage;}catch(err){}
